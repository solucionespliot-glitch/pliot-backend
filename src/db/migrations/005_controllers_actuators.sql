-- Migration: 005_controllers_actuators
...
CREATE TABLE IF NOT EXISTS controllers (
  ...
  mode                       TEXT        DEFAULT 'auto' CHECK (mode IN ('auto','manual','mixed')),
  ...
  sync_status                TEXT        DEFAULT 'pending' CHECK (sync_status IN ('synced','pending','outdated','error')),
...
CREATE TABLE IF NOT EXISTS irrigation_system_specs (
  id                 UUID    PRIMARY KEY DEFAULT gen_random_uuid(),
  actuator_id        UUID    UNIQUE NOT NULL REFERENCES actuators(id),
